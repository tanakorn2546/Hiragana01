...
<style>
    ...
    .app-header-icon {
        ...
        margin: 0 auto 15px auto; box-shadow: 0 10px 25px rgba(211, 47, 47, 0.2);
        border: 5px solid #ffffff;
    }
    h1 { color: #D32F2F !important; text-align: center; font-weight: 800; }
    .result-box {
        background-color: #FFEBEE; padding: 20px; border-radius: 15px; 
        border: 2px solid #D32F2F; margin-bottom: 20px; text-align: center;
    }
</style>
...
def get_student_work(work_id):
    try:
        ...
        # ดึง Path รูปและสถานะเดิมจากตาราง progress
...
def update_student_progress(work_id, result, confidence):
    try:
        conn = init_connection()
        cursor = conn.cursor()
        # อัปเดตผล AI กลับไปที่ตาราง progress
        sql = "UPDATE progress SET ai_result = %s, ai_confidence = %s, status = 'analyzed' WHERE id = %s"
...
# ฟังก์ชันสำหรับ Gallery Mode (ดูรูปทั่วไป)
...
# --- Helper: โหลด Model (รองรับ Streamlit ทุกเวอร์ชัน) ---
# ตรวจสอบเวอร์ชันเพื่อเลือกใช้ cache ที่ถูกต้อง
...
# --- Helper: รับค่า URL Parameter (รองรับทุกเวอร์ชัน) ---
...
# รับค่า work_id จาก URL (ส่งมาจาก teacher.php)
...
# ==========================================
if target_work_id:
    st.info(f"📋 โหมดตรวจงานนักเรียน (Work ID: {target_work_id})")
    
    # 1. ดึงข้อมูลจาก DB
    work_data = get_student_work(target_work_id)
    
    if work_data and work_data['image_path']:
        # 2. สร้าง URL รูปภาพ (ต่อ Domain เข้ากับ Path ใน DB)
        # ตัวอย่าง DB: uploads/works/uid1_a_17000.png -> ต้องเติม https://www.cedubru.com/ ข้างหน้า
        ...
        st.warning("❌ ไม่พบข้อมูลงานนี้ หรือยังไม่มีไฟล์รูปภาพในระบบ")

# ==========================================
# 🅱️ MODE 2: Gallery Browser (ไม่มี ID - ดูรูปทั่วไป)
# ==========================================
else:
    st.write("---")
    c1, c2, c3 = st.columns([0.1, 3, 0.1])
    with c2:
        filter_option = st.radio("📂 เลือกดูข้อมูล (Gallery Mode - culantro_images):", ["ทั้งหมด", "ยังไม่ตรวจ"])
